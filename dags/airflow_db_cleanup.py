...
from airflow.models import DAG, DagModel, DagRun, Log, Variable, XCom
...
DEFAULT_MAX_DB_ENTRY_AGE_IN_DAYS = int(Variable.get("airflow_db_cleanup__max_db_entry_age_in_days", 30))
PRINT_DELETES = False
    ...
    DATABASE_OBJECTS.append(
        {
            ...
            "age_check_column": TaskFail.dag_id,
            "keep_last": False,
            "keep_last_filters": None,
            "keep_last_group_by": None,
        }
...
def cleanup_sessions():
    ...
    try:
        ...
        before = len(session.execute(text("SELECT * FROM session WHERE expiry < now();")).mappings().all())
        session.execute(text("DELETE FROM session WHERE expiry < now();"))
        after = len(session.execute(text("SELECT * FROM session WHERE expiry < now();")).mappings().all())
