{{
    config(
        materialized='view',
        schema='edw_fpna',
        alias='VIEW_FPNA_PROFIT_VS_CLOSING_LODGING_M',
        tags=[ 'MART', 'FPNA', 'VIEW' ]
    )
}}

/*
  숙박 월별 CGMV/Revenue를 FPNA(Profit) vs 매관(Closing) 기준으로 나란히 비교한다.
  그레인: 월 x 파트너 x 정산타입

  원천:
  - Profit: MART_FPNA_NONAIR_PROFIT_M (FPNA_TYPE_LV_1 = '숙박')
  - Closing: MART_FPNA_CONFIRMATION_CLOSING_D (전체 NONAIR, vertical 컬럼 없음)

  이벤트월 기준 (Profit vs Closing 차이 발생 원인):
  - Profit CONFIRM: CONFIRM_KST_DATE 기준 (예약 시스템 확정일, orders.reservations.confirmed_at)
  - Profit CANCEL:  REFUND_MONTH 기준 (정산 시스템 확정일, confirmation_product_closing.confirmation_date)
  - Closing 결제/환불: BASIS_DATE 기준 (정산 시스템 확정일, confirmation_product_closing.confirmation_date)
  - CANCEL/환불은 양쪽 모두 정산 확정일 기준이므로 월 배정 차이 최소화
  - CONFIRM/결제는 Profit=예약 확정일, Closing=정산 확정일로 기준이 다름
    → 예약 확정(11월)과 정산 확정(12월)이 다른 월에 걸리는 건에서 차이 발생

  Closing vertical 제약:
  - Closing에는 vertical(숙박/T&A/렌터카) 구분 컬럼이 없어 파트너의 전체 NONAIR가 합산됨
  - T&A/마이팩 겸업 파트너는 Closing > Profit 차이 발생
